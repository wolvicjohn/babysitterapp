...
class _UserViewLocationState extends State<UserViewLocation> {
  ...
  double _circleRadiusInKm = 1.0;
  bool _isSliderActive = false;
  double _convertKmToMapRadius(double kilometers) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: GestureDetector(
        ...
        onTap: () {
          //lose focus when tapping outside the slider
          if (_isSliderActive) {
            setState(() {
    ...
    return CircleLayer(
      circles: [
        CircleMarker(
          ...
          borderColor: Colors.blue,
          borderStrokeWidth: 2,
          radius: _convertKmToMapRadius(_circleRadiusInKm),
        ),
      ],
    );
  }

  Widget _buildRadiusSliderToggle() {
    return Positioned(
      top: 20,
      right: 20,
      child: Column(
        children: [
          ...
          if (_isSliderActive)
            RotatedBox(
              quarterTurns: 3,
              child: Slider(
                value: _circleRadiusInKm,
                min: 1.0,
                max: 500.0,
                divisions: 100,
                label: '${_circleRadiusInKm.toStringAsFixed(1)} km',
                onChanged: (value) {
                  setState(() {
                    _circleRadiusInKm = value;
                  });
                },
              ),
            ),
          ...
          ClipOval(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                shape: BoxShape.circle,
                image: babysitter['img'] != null && babysitter['img'].isNotEmpty
                    ? DecorationImage(
                  image: AssetImage(babysitter['img']),
                  fit: BoxFit.cover,
                )
                    : null,
              ),
              child: babysitter['img'] != null && babysitter['img'].isNotEmpty
                  ? Image.asset(
                babysitter['img'],
                width: 30,
                height: 30,
